...
Route::middleware('auth')->prefix("admin")->group(function(){
    ...
    Route::get("/teste/arrumar",function(){
        /*
        $clt = \App\Models\ComissoesCorretoresLancadas
            ::whereRaw(" not in(2,3,26,36,57,18,24) and plano_id = 3")
            ->get();
        dd($clt);
        ...
        $dados = DB::table('comissoes_corretores_lancadas')
            ...
            ->selectRaw("comissoes_corretores_lancadas.id,contratos.valor_adesao")
            ->get();
        */
        ...
        /*
        foreach($dados as $v) {
        //     echo $v->valor_adesao."<br />";
            $alt = \App\Models\ComissoesCorretoresLancadas::where("id",$v->id)->first();
            $alt->valor = 0;
            $alt->save();
        }
        */
        ...
        */
        ...
        /*
         $dados = DB::table('comissoes_corretores_lancadas')
            ->join('comissoes','comissoes.id',"=",'comissoes_corretores_lancadas.comissoes_id')
            ->join('contratos','contratos.id',"=",'comissoes.contrato_id')
            ->whereRaw("comissoes_id IN(SELECT id FROM comissoes where user_id not in(2,3,26,36,57,18,24) and plano_id = 3)")
            //->where("parcela",5)
            ->selectRaw("comissoes_corretores_lancadas.id,contratos.valor_adesao")
            ->get();
            
        foreach($dados as $d) {
            $alt = DB::table('comissoes_corretores_lancadas')->where("id",$d->id);
            $alt->update(["valor"=>0]);
            //$alt->valor = 0;
            //$alt->save();
        }        
        ...
        // $cc = DB::table('comissoes_corretores_default')
        //      ->where("plano_id",3)
        //      ->where("administradora_id",3)
        //      ->get();
             
          /*   
         $dados = \App\Models\ComissoesCorretoresDefault
            ::where("plano_id",3)
            ->where("administradora_id",3)
            ->where("parcela",2)
            ->first()->valor;     
        ...
        foreach($comissoesLancadas as $d) {
            ...
            DB::table('comissoes_corretores_lancadas')->where("id",$d->id)->update(["valor"=>$valor]);
            //DB::table('comissoes_corretores_lancadas')->update(["valor"=>$valor])->where()
            //$alt->valor = ($contrato * $dados) / 100;
            //$alt->save();
        ...
        */
        ...
        foreach($dados as $c) {
            ...
            //if($c->valor != 0) {
                ...
                dd($comissoesLancadas);
                
                
                // foreach($comissoesLancadas as $cc) {
                //     $cc->valor = ($cc->valor_adesao * $c->valor) / 100;
                //     $cc->save();
                // }
            //}
        ...
        */
        ...
        foreach($cc as $c) {
            ...
            $alt = DB::table('comissoes_corretores_lancadas')
                ->where("parcela",$c->id)
                ->where
        ...
        */
        ...
        /*foreach($dados as $d) {
            ...
            //foreach($co as $c) {
                 ...
                 //echo $alt->id."<br />";
                 //echo $alt->valor."<br />";
        //     //     //$alt->valor = ($v->valor_adesao * $c->valor) / 100;
        //     //     //$alt->save();     
                // $alt
                // $d->valor = ($v->valor_adesao * $c->valor) / 100;
                // $
            //}
        }*/
        // echo "</pre>";
        ...
        // $co = DB::table('comissoes_corretores_default')
        //     ->where("plano_id",3)
        //     ->where("administradora_id",3)
        //     ->get();
        ...
        //     echo $v->
        //     // foreach($co as $c) {
        //     //     echo ($v->valor_adesao * $c->valor) / 100;
        //     //     echo "<br />";
        //     //     //echo $v->valor_adesao."<br />";
        //     //     //$alt = \App\Models\ComissoesCorretoresLancadas::where("id",$v->id)->first();
        //     //     //$alt->valor = ($v->valor_adesao * $c->valor) / 100;
        ...
        /*
        select count(*) from comissoes_corretores_lancadas 
        ...
        where comissoes_id 
        IN(SELECT id FROM comissoes where user_id not in(2,3,26,36,57,18,24) and plano_id = 3);
        ...
        SELECT count(*) FROM comissoes_corretores_lancadas 
        inner join comissoes on comissoes.id = comissoes_corretores_lancadas.comissoes_id 
        where comissoes_id IN (SELECT id FROM comissoes where administradora_id = 3) and comissoes.user_id not in (2,3,18,24,26,36,57);
        */
        
        
        
        /*
        $comissoes_configuradas_corretor = \App\Models\ComissoesCorretoresConfiguracoes
        ::where("plano_id",3)
        ->where("administradora_id",3)
        ->where("user_id",15)
        ->get();
        dd(count($comissoes_configuradas_corretor));
