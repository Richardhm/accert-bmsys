...
//        $comissa_lancada->save();
    ...
    public function criarPDFUser(Request $request)
    {
        ...
        $total_empresarial = 
        DB::select("SELECT	SUM((comissoes_corretores_lancadas.valor)) AS total FROM comissoes_corretores_lancadas INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id WHERE comissoes_corretores_lancadas.status_financeiro = 1 AND comissoes_corretores_lancadas.status_apto_pagar = 1 AND comissoes.user_id = {$id} AND MONTH(data_baixa_finalizado) = {$mes} AND YEAR(data_baixa_finalizado) = {$ano} AND valor != 0 AND comissoes.plano_id != 1 AND comissoes.plano_id != 3")[0]->total;
